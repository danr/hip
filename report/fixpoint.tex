...
be defined in Haskell as \hs{fix}:
...
\hs{fix}: this is a fixed point of the equation, futhermore it is the
...
\ref{code:mapfix} is immediate: \hs{fix map'} evaluates to
\hs{map' (fix map')} by the definition of \hs{fix}, and this means
that the recursive function \hs{m} be \hs{fix map'}, so by
construction it is equal to the originial deinition of \hs{map}.

Generally, for a function \hs{f} with $n \leq 0$ arguments $\hs{x}_1$
...
of \hs{fix}. We can now state its inference rule:
...
It is crucial that $P$ is admissible to avoid deriving nonsense. This
section gives a simple example of such nonsense.  Recall the
definition of the infinite natural number:
...
translated to logic and rewritten in terms of $\unfix{}$ and
...
\noindent
Base case: $\bot \neq \fn{succ}(\bot)$ is true by the axioms of
disjoint constructors.

\noindent
Step case: assume
$\unfix{\fn{inf}} \neq \fn{succ}(\unfix{\fn{inf}})$.
Injectivity of $\fn{succ}$ gives
$\fn{succ}(\unfix{\fn{inf}}) \neq \fn{succ}(\fn{succ}(\unfix{\fn{inf}}))$.
By the definition of $\tofix{\fn{inf}}$, this is equal to the goal
$\tofix{\fn{inf}} \neq \fn{succ}(\tofix{\fn{inf}})$.
