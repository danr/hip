...
\noindent
Could the elegant simplicity of bind possibly could obey the three
monad laws as stated by \cite{essenceoffp}? Referential transparency
in Haskell allows equational reasoning, but proving the monad laws by
...
reports this after a few seconds\footnote{The presentation
  format is slightly rewritten for your viewing pleasure.}:
...
\begin{enumerate}
\item a translation from Haskell to \emph{first order logic},
\item instantiating the properties with different \emph{induction techniques}, and
\item running \emph{automated theorem provers} on the generated theories.
\end{enumerate}
