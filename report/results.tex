...
\ref{tbl:testsuite}, and available in the project's
repository\footnote{See the \hs{testsuite} directory in
  \url{http://github.com/danr/hip/}}, but some parts and design
decisions are highlighted in the rest of this section.
...
\hs{MonadState}, defined without a \hs{newtype}.
...
list. Insertion sort works with a helper function \hs{insert} that
inserts an element in a sorted list at its right position. To prove
properties about insertion sort lemmas for \hs{insert} are needed.
...
The stats from theorems are considered in Figure
