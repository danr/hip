...
\hs{monospace} and their counterpart in first order logic as $\fn{this}$.}:
\begin{equation*}
...
\emph{disjoint constructor axioms}. For the data type \hs{Tree}, only
...
non-terminating functions. For \hs{top}, the idea is to add an axiom
so that $\fn{top}$ applied to anything that is not a \hs{Fork} is
