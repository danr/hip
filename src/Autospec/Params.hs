...
defParams = Params
  ...
  , processes   = 4       &= help "Number of prover processes (default 4)" &= name "p" &= groupname "Proving settings"
  ...
  , inddepth    = 1       &= help "Max depth to do structural induction (default 1)" &= groupname "Structural induction"
  ...
  , fpimax      = 25      &= help "Maximum number of lifted functions (default 25)" &= groupname "Fixpoint induction"

  , tptp        = False   &= help "Generate tptp output and terminate" &= name "f"  &= groupname "Debug"
